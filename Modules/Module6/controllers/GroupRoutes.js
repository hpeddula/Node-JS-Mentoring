...
router.put('/updateGroup/:id',authenTicateJWT,async(req,res)=>{
    const { id } = req.params;
    const { permissions,name } = req.body;
    const { isValid, message } = permissionValidator(permissions);
    try{
...
router.delete('/deleteGroup/:id',authenTicateJWT,async(req,res)=>{
    const { id } = req.params;
    try{
...
router.post('/addUsersToGroup/:groupId',authenTicateJWT,async(req,res)=>{
